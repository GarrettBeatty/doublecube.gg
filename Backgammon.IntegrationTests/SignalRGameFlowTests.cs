...
namespace Backgammon.IntegrationTests
{
    ...
    public class SignalRGameFlowTests : IClassFixture<CustomWebApplicationFactory>
    {
        ...
        public async Task CanCreateJoinAndPlayGame()
        {
            ...
            player1.Closed += async (error) => { Console.WriteLine($"Player1 closed: {error}"); await Task.CompletedTask; };
            player2.Closed += async (error) => { Console.WriteLine($"Player2 closed: {error}"); await Task.CompletedTask; };
            player1.Reconnecting += async (error) => { Console.WriteLine($"Player1 reconnecting: {error}"); await Task.CompletedTask; };
            player2.Reconnecting += async (error) => { Console.WriteLine($"Player2 reconnecting: {error}"); await Task.CompletedTask; };
